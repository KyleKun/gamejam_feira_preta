...
class NPC1 extends GameDecoration with Sensor {
  ...
  void update(double dt) {
    if ((gameRef.player as Faxineira).trashCount == 3 && !_completedTask) {
      _completedTask = true;
      (gameRef.player as Faxineira).mission = 'Falar com Léo';
  ...
  void _giveTrashTask() {
    TalkDialog.show(
      ...
      (gameRef.player as Faxineira).mission = ' Coletar lixo';
    });
  }
      ...
      Future.delayed(Duration(milliseconds: (1 * 1000)), () {
        TalkDialog.show(
          ...
          Future.delayed(Duration(milliseconds: (2 * 1000)), () {
            (gameRef.player as Faxineira).sofreu = true;
          });
